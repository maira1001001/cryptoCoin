...
const { account2, id, provider, contractAddress } = require("./constants");
...
const getMinter = (web3) =>
  web3.eth.accounts.privateKeyToAccount(process.env.PRIVATE_KEY_1); // Account1: 0x78E0081C4F7051bD29Ce6891F65e9525eE36d8c3

const mintRDVCoins =
  ({ contractInstance, web3, minter, contractAddress }) =>
...
const sendRDVCoins =
  ({ contractInstance, web3, contractAddress }) =>
...
const init = async () => {
  const web3 = await connectToNetwork(provider);
  ...
  const mint = mintRDVCoins({ contractInstance, web3, minter, contractAddress });

  await mint({ receiver: account2, amount: 150 });

  console.log("Total Balance: ", await getTotalBalance(contractInstance));

  const send = sendRDVCoins({ contractInstance, web3, contractAddress });
